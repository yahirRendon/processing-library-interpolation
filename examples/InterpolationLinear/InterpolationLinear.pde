/******************************************************************************
 Project:  example of interpolation library using linear interpolation (lerp)
           and inverse interpolation (norm).
 ...
 Parameter Guide:
          ...
          lerpInverse(int a, int b, float v)        
 ******************************************************************************/
...
void draw() {
  ...
  float interpValue = interpolation.lerp(0, 200, t);
  ...
  // color going between 0 and 200 given linear interpolation defined above
  fill(interpValue);
  ...
  for(int i = 0; i <= 200; i+= 25, gap++) {
   ...
   float inverseValue = interpolation.lerpInverse(0, 200, i);
   ...
   // feed inverseValue into linear interpolation to confirm they match i
   text(interpolation.lerp(0, 200, inverseValue), 300, 220 + (gap * 20));
  }
}
